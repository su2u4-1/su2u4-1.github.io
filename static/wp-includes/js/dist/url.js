...
/******/ 			Object.defineProperty(exports, '__esModule', { value: true });
	...
	(() => {
		 ...
		 * @example
		 * ```js
		 * const isURL = isURL( 'http://wordpress.org' ); // true
		 * ```
		 *
		 * @see http://url.spec.whatwg.org/
		 * @see http://url.spec.whatwg.org/#valid-url-string
		 *
		 ...
		 * const protocol2 = getProtocol( 'http://wordpress.org' ); // 'http:'
		 * ```
		 *
		 ...
		 * const isNotValid = isValidProtocol( 'http :' ); // false
		 * ```
		 *
		 ...
		 * @example
		 * ```js
		 * const authority1 = getAuthority( 'http://wordpress.org/help/' ); // 'wordpress.org'
		 * const authority2 = getAuthority( 'http://localhost:8080/test/' ); // 'localhost:8080'
		 * ```
		 *
		 ...
		 * const path2 = getPath( 'http://wordpress.org/help/faq/' ); // 'help/faq'
		 * ```
		 *
		 ...
		 * const pathAndQueryString2 = getPathAndQueryString( 'http://wordpress.org/help/faq/' ); // '/help/faq'
		 * ```
		 *
		 ...
		 * const fragment2 = getFragment( 'http://wordpress.org#another-fragment?query=true' ); // '#another-fragment'
		 * ```
		 *
		 ...
		 * @example
		 * ```js
		 * const foo = getQueryArgs( 'http://wordpress.org?foo=bar&bar=baz' );
		 * // { "foo": "bar", "bar": "baz" }
		 * ```
		 *
		...
		function getQueryArgs(url) {
			return (getQueryString(url) || ''
				...
				//
				// See: http://tools.ietf.org/html/rfc1866#section-8.2.1
		 ...
		 * @example
		 * ```js
		 * const newURL = addQueryArgs( 'http://google.com', { q: 'test' } ); // http://google.com/?q=test
		 * ```
		 *
		 ...
		 * @example
		 * ```js
		 * const foo = getQueryArg( 'http://wordpress.org?foo=bar&bar=baz', 'foo' ); // bar
		 * ```
		 *
		 ...
		 * @example
		 * ```js
		 * const hasBar = hasQueryArg( 'http://wordpress.org?foo=bar&bar=baz', 'bar' ); // true
		 * ```
		 *
		 ...
		 * @example
		 * ```js
		 * const newUrl = removeQueryArgs( 'http://wordpress.org?foo=bar&bar=baz&baz=foobar', 'foo', 'bar' ); // http://wordpress.org?baz=foobar
		 * ```
		 *
		 ...
		 * @example
		 * ```js
		 * const displayUrl = filterURLForDisplay( 'http://www.wordpress.org/gutenberg/' ); // wordpress.org/gutenberg
		 * const imageUrl = filterURLForDisplay( 'http://www.wordpress.org/wp-content/uploads/img.png', 20 ); // …ent/uploads/img.png
		 * ```
		 *
		 ...
		 * Prepends "http://" to a url, if it looks like something that is meant to be a TLD.
		 *
		 * Note: this will not replace "http://" with "http://".
		 *
		 * @param {string} url The URL to test.
		 *
		 * @example
